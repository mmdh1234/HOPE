...
TARGETS_3x3_CENTER = snake_order(create_targets(M, M, 1-M, 1-M, 3, 3), 3, 3)
...
while cap.isOpened():
    ...
    if gmm_calib['running']:
        ...
        if has_face and not is_message_visible:
            ...
            'head_vertical','head_rotation','forward_backward', 'head_translation'
                }
        ...
        if not is_message_visible and gmm_calib['phase'] in ['center_gaze', 'left_gaze', 'right_gaze']:
            if has_face and gmm_calib['idx'] < len(gmm_calib['targets']):
                ...
                if gmm_calib['idx'] < len(gmm_calib['targets']):
                    ...
                    cv2.circle(final_image, (tx, ty), 20, (225, 255, 0), -1)
        ...
        if gmm_calib['phase'] == 'intro' and current_time > gmm_calib['message_end_time']:
            ...
            if gmm_calib['idx'] >= len(gmm_calib['targets']):
                gmm_calib['message'] = "시선은 화면, 고개를 위아래로 천천히 움직여주세요."
                gmm_calib['message_end_time'] = current_time + 2.0
        ...
        elif gmm_calib['phase'] == 'head_translation' and not is_message_visible and has_face:
            if current_time > gmm_calib['phase_until']:
                gmm_calib['message'] = "얼굴을 앞뒤로 천천히 움직여주세요."
                gmm_calib['message_end_time'] = current_time + 2.0
                gmm_calib['phase'] = 'message_before_forward_backward'
                gmm_calib['current_guide_pos'] = (int(screen_width / 2), int(screen_height / 2))

        elif gmm_calib['phase'] == 'message_before_forward_backward' and current_time > gmm_calib['message_end_time']:
            gmm_calib['phase'] = 'forward_backward'
            gmm_calib['phase_until'] = current_time + 5.0

        elif gmm_calib['phase'] == 'forward_backward' and not is_message_visible and has_face:
            if current_time > gmm_calib['phase_until']:
                gmm_calib['message'] = "얼굴은 화면 안, 시선은 화면 밖(옆/아래 등)을 바라봐주세요."
                gmm_calib['message_end_time'] = current_time + 2.0
                gmm_calib['phase'] = 'message_before_look_away'
                gmm_calib['current_guide_pos'] = (int(screen_width / 2), int(screen_height / 2))

        elif gmm_calib['phase'] == 'message_before_look_away' and current_time > gmm_calib['message_end_time']:
            gmm_calib['phase'] = 'look_away'
            gmm_calib['phase_until'] = current_time + gmm_calib['neg_collect_secs']
        
        elif gmm_calib['phase'] == 'look_away' and not is_message_visible:
            if current_time > gmm_calib['phase_until']:
    ...
    if gmm_calib['phase'] == 'look_away':
        remaining = max(0.0, gmm_calib['phase_until'] - current_time)
        final_image = draw_text_on_image(
            final_image,
            f"이탈 수집 남은 시간: {remaining:.1f}s",
            pos=(int(screen_width/2 - 220), 80),
            font_scale=1.2,
            color=(255, 0, 0)
        )
...
output_data = {
    ...
    'image_paths': samples_Y,
    'feature_schema_id': 'v3_iris_7_features',
    'feat_names': ['ear_l','ear_r','pitch','gL_h','gR_h','gL_v','gR_v']
}

with open(os.path.join(DATA_FOLDER, 'features.json'), 'w', encoding='utf-8') as f:
    json.dump(output_data, f, indent=4, ensure_ascii=False)

print(f"\n총 {len(samples_X)}개의 특징 샘플과 {len(samples_Y)}개의 이미지가 저장되었습니다.")
print("labels: 1 =", sum(l==1 for l in samples_labels), "/ 0 =", sum(l==0 for l in samples_labels))
print(f"데이터는 '{os.path.abspath(DATA_FOLDER)}' 폴더에 저장되었습니다.")
