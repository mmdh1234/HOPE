...
const StatusIndicator = styled.div`
    ...
    padding: 0.5rem;
    border-radius: 8px;
    background-color: ${({ isConcentrating }) => isConcentrating ? '#e6f7ff' : '#fff1f0'};
    color: ${({ isConcentrating }) => isConcentrating ? '#1890ff' : '#f5222d'};
    font-weight: bold;
`;
...
const ConcentrationChecker = () => {
    ...
    useEffect(() => {
        const handleModelStart = async () => {
            console.log("▶️ [Component] ConcentrationChecker가 마운트되어 모델 시작을 시도합니다.");
            try {
                const response = await startModel();
                console.log("✅ [Component] 모델 시작 API 호출 성공!", response.data);
            } catch (error) {
        ...
        return () => {
            const handleModelStop = async () => {
                console.log("⏹️ [Component] ConcentrationChecker가 언마운트되어 모델 중지를 시도합니다.");
                try {
                    const response = await stopModel();
                    console.log("✅ [Component] 모델 중지 API 호출 성공!", response.data);
                } catch (error) {
                    console.error("❌ [Component] 모델 중지 API 호출 실패:", error);
                }
            };
            handleModelStop();
        };
            ...
            if (video && video.currentTime !== lastVideoTime) {
                ...
                if (results.faceLandmarks && results.faceLandmarks.length > 0) {
                    ...
                    if (nose.x < 0.2 || nose.x > 0.8 || nose.y < 0.2 || nose.y > 0.8) {
                        setStatus('집중 필요'); setIsConcentrating(false);
                    } else {
                        setStatus('집중 중'); setIsConcentrating(true);
                    }
                } else {
                    setStatus('얼굴을 보여주세요'); setIsConcentrating(false);
                }
            }
